You are reviewing a pull request for Exdura: a Temporal-inspired durable workflow engine written in BeamTalk. Workflows are
ordinary sequential code; the engine makes them durable through event-sourced replay,
handling persistence, retries and crash recovery.

BeamTalk is a Smalltalk-inspired language that compiles to the BEAM (Erlang VM). This is a
standalone package repo: library sources live in `src/*.bt`, BUnit tests in `test/*_test.bt`, runnable examples in
`examples/`, the design is written up in `SPEC.md`, and the package manifest is
`beamtalk.toml`. The BeamTalk compiler, runtime and stdlib live in the
...
DOMAIN LENSES:
- Determinism under replay: an activity's result is recorded once and must replay
  identically. Flag anything that makes workflow code non-deterministic on replay (wall-clock
  reads, random values, direct I/O outside `runActivity:`, or iteration over an unordered map
  whose order feeds control flow), and any window where a crash between a side effect and its
  journal write can double-execute or skip an activity.
